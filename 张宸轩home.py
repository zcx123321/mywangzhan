...
def page_2():
    ...
    name = st.selectbox('我是……', ['游客','张宸轩','王宇轩'])
    if name!='游客':
        if st.toggle('实时密码'):
            time_a=time.localtime()
            password = st.text_input('请输入密码')
            if name=='张宸轩':
                if password!=str(time_a.tm_min+time_a.tm_hour) and password!='':
                    st.write('密码错误请重新输入')
                    flag=False
                elif password==str(time_a.tm_min+time_a.tm_hour):
                    st.write('验证通过')
                    flag=True
            if name=='王宇轩':
                if password!=str(time_a.tm_min) and password!='':
                    st.write('密码错误请重新输入')
                    flag=False
                elif password==str(time_a.tm_min):
                    st.write('验证通过')
                    flag=True
        else:
            password = st.text_input('请输入密码')
...
def page_6():
    ...
    for i in range(len(words_list)):
        words_list[i]=words_list[i].split('#')
    if st.toggle('随机抽词'):
        number7, number8 = st.slider('背词典中从几到几的词：', 1, len(words_list), (1,len(words_list)))
        if number7==number8:
            st.write('不要偷懒！！！')
            number7=1
            number8=len(words_list)
        number6 = st.slider('抽几个词:', 1, number8-number7+1,1)
        number5 = st.slider('一个词背几秒:', 1, 60,10)
        lst=words_list[number7-1:number8+1]
        if st.button('开始背词'):
            words = st.progress(0, '准备开始背词！')
            for j in range(0,number6):
                a=lst[r.randint(0,len(lst))]
                for i in range(number5, 0, -1):
                    time.sleep(1)
                    words.progress(i*int((100/number5)),str(a[1]+'，意思为'+a[2]))
            words.empty()
            st.write('结束了')
        ...
        number5 = st.slider('一个词背几秒:', 1, 60,10)
        if st.button('开始背词'):
            lst=words_list[number3-1:number4+1]
            words = st.progress(0, '准备开始背词！')
